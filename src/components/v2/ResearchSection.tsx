...
const JwtLogo = () => (
  <svg className="w-[84px] xs:w-[101px] sm:w-[120px] h-auto" viewBox="0 0 341 49" fill="none" xmlns="http://www.w3.org/2000/svg">
    {JWT_PATHS.map((d, idx) => (
      <path key={idx} d={d} fill="white" />
    ))}
  </svg>
);
...
const ResearchSection = () => {
  ...
  const renderLoop = () => {
    ...
    for (let i = 0; i < cardCount; i++) {
      ...
      card.style.transform = `translateY(${y.toFixed(2)}px) translateZ(${z.toFixed(2)}px) rotateX(${totalRotX.toFixed(2)}deg) rotateY(${activeTiltY.toFixed(2)}deg) rotateZ(-3deg)`;
    }
  };
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="absolute left-4 sm:left-6 bottom-4 sm:bottom-5 z-20 flex flex-col gap-0.5 sm:gap-1 text-left" style={{ fontFamily: '"JetBrains Mono", monospace' }}>
                          ...
                          <div className="font-mono text-[7px] sm:text-[9px] font-medium text-white/70 tracking-wide flex items-center gap-2 select-none whitespace-nowrap">
