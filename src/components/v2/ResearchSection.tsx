...
const CARD_DETAILS = [
  ...publications.map(p => ({ number: p.title.substring(0, 36) + (p.title.length > 36 ? '...' : ''), name: p.journal, cvv: p.date })),
  ...certifications.map(c => ({ number: c.title.substring(0, 36) + (c.title.length > 36 ? '...' : ''), name: c.issuer, cvv: c.level || "CERT" }))
];

const ResearchSection = () => {
  const cardCount = CARD_DETAILS.length;
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      // 2. Calculate Typography Metrics
  ...
  return (
    <section
      id="research"
      className="font-kanit rounded-t-[40px] sm:rounded-t-[50px] md:rounded-t-[60px] -mt-10 sm:-mt-12 md:-mt-14 relative z-40 overflow-hidden"
      style={{
        background: "#111111", 
        height: "100vh",
        minHeight: "800px",
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-50 p-6 md:p-10 pointer-events-none">
        <div className="max-w-6xl mx-auto w-full">
          <FadeIn delay={0} y={20}>
            <p className="flex items-center gap-2 font-medium uppercase tracking-[0.25em] text-[#D7E2EA]/50 text-xs mb-3">
      ...
      <div className="absolute inset-0 text-white flex items-center justify-center select-none" style={{ background: "transparent" }}>
...
423:                   // Back face slice
424:                   if (isBackFace) {
425:                     const backBorderStyle = "border border-white/15";
426:                     const details = CARD_DETAILS[i % CARD_DETAILS.length];
427:                     return (
428:                       <div
429:                         key={layerIdx}
430:                         className={`absolute inset-0 rounded-[16px] ${backBorderStyle} pointer-events-none overflow-hidden`}
431:                         style={{
432:                           backgroundColor: baseBgColor,
433:                           transform: `translateZ(${zOffset}px) rotateX(180deg)`,
434:                           backfaceVisibility: 'hidden',
435:                           boxShadow: 'inset 0 1px 1px rgba(255,255,255,0.15)',
436:                         }}
437:                       >
438:                         {/* Render Video with premium 16px blur on the back face of the card */}
439:                         <div className="absolute inset-0 pointer-events-none" style={{ filter: 'blur(16px)', transform: 'scale(1.15)' }}>
440:                           <video
441:                             src={videoSrc}
442:                             autoPlay
443:                             loop
444:                             muted
445:                             playsInline
446:                             className="absolute inset-0 w-full h-full object-cover"
447:                           />
448:                         </div>
449: 
450:                         {/* Premium Real Magnetic stripe */}
451:                         <div className="absolute left-0 right-0 top-4 sm:top-5 h-7 sm:h-9 bg-black/85 backdrop-blur-md z-10" />
452: 
453:                         {/* Card holder info and details on the bottom-left */}
454:                         <div 
455:                           className="absolute left-4 sm:left-6 bottom-4 sm:bottom-5 z-20 flex flex-col gap-0.5 sm:gap-1 text-left"
456:                           style={{ fontFamily: '"JetBrains Mono", monospace' }}
457:                         >
458:                           {/* Card Number */}
459:                           <div className="font-mono text-[10px] sm:text-[12px] font-medium tracking-[0.14em] text-white select-none">
460:                             {details.number}
461:                           </div>
462:                           {/* Owner & CVV */}
463:                           <div className="font-mono text-[7px] sm:text-[9px] font-medium text-white/70 tracking-wide flex items-center gap-2 select-none whitespace-nowrap">
464:                             <span className="uppercase">{details.name}</span>
465:                             <span className="text-white/40 font-light">•</span>
466:                             <span>{details.cvv}</span>
467:                           </div>
468:                         </div>
469:                       </div>
470:                     );
...
482: };
483: 
484: export default ResearchSection;
