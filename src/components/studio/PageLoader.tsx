    ...
    <AnimatePresence onExitComplete={onDone}>
      {!exiting && (
        <motion.div
          ...
          className="fixed inset-0 z-[120] flex flex-col items-center justify-center gap-8 rounded-b-[2rem] bg-ink px-6 text-white"
        >
          <div className="flex flex-col items-center gap-5 text-center">
            <div className="flex items-center gap-3 text-2xl font-semibold sm:text-3xl">
              <LogoMark className="text-[1.875rem] text-[#cf8047]" />
              Saran Jaya Thilak
            </div>
          ...
          <div className="flex w-[min(22rem,72vw)] flex-col gap-3">
            <div className="h-px w-full bg-white/15">
              <div className="h-full bg-[#cf8047] transition-[width] duration-100 ease-out" style={{ width: `${progress}%` }} />
            </div>
            <div className="flex items-center justify-between text-xs font-medium uppercase tracking-[0.05em] text-white/45">
              <span>Loading</span>
              <span className="tnum text-white/80">{String(progress).padStart(3, "0")}</span>
            </div>
          </div>
        </motion.div>
      )}
