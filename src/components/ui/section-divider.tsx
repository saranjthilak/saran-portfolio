...
const SectionDivider = ({ variant = "cyan", flip = false }: SectionDividerProps) => {
  ...
  const particles = useMemo(() => {
    ...
    return Array.from({ length: 7 }, (_, i) => ({
      id: i,
      xOffset: (sRand(seed * 10 + i) - 0.5) * 300,
      yOffset: (sRand(seed * 10 + i + 50) - 0.5) * 30,
  ...
  if (reduce) {
    return (
      <div
        ref={ref}
        className="relative w-full h-px pointer-events-none select-none"
    ...
    >
      ...
      <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 h-px">
        ...
        <motion.div
          ...
          style={{
            background: `linear-gradient(90deg, transparent 5%, ${colors.primary}30 20%, ${colors.primary}60 50%, ${colors.secondary ?? colors.primary}30 80%, transparent 95%)`,
          }}
          initial={{ scaleX: 0 }}
        ...
        <motion.div
          ...
          style={{
            background: `linear-gradient(90deg, transparent 10%, ${colors.primary}80 30%, ${colors.primary} 50%, ${colors.secondary ?? colors.primary}80 70%, transparent 90%)`,
      ...
      {/* ── Flanking data readouts (tiny decorative text) ── */}
      <motion.span
        className="absolute top-1/2 -translate-y-1/2 font-mono text-[7px] tracking-[0.3em] uppercase select-none"
        style={{
          left: "8%",
